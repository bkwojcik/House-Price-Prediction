...
fwrite(train[,.(id,UniqueCarrier, Origin,DepDelay)],'./project/volume/data/raw/train.csv')
fwrite(test[,.(id,DepDelay)],'./project/volume/data/raw/solution.csv')
test$DepDelay<-1
fwrite(test[,.(id,UniqueCarrier, Origin,DepDelay)],'./project/volume/data/raw/test.csv')
